...
def load_checkpoint(models, model_names, dirname, epoch=None, optimizers=None, optimizer_names=None, strict=True, device=None):
    if len(models) != len(model_names) or (optimizers is not None and len(optimizers) != len(optimizer_names)):
        raise ValueError('Number of models, model names, or optimizers does not match.')
    ...
    for model, model_name in zip(models, model_names):
        ...
        if epoch is not None and epoch > 0:
            filename = f'{epoch}_' + filename
        model.load_state_dict(torch.load(os.path.join(dirname, filename), map_location=device), strict=strict)
...
def boxLoss(box_feature, gt_box_feature):
    ...
    pred_reweight = get_surface_reweighting_batch(box_feature[:, 3:6], unit_cube.size(0))
    gt_box_pc = transform_pc_batch(unit_cube, gt_box_feature)
    gt_reweight = get_surface_reweighting_batch(gt_box_feature[:, 3:6], unit_cube.size(0))
...
def compute_box_diff(box1, box2):
    center_diff = box2[:3] - box1[:3]
    size_diff = box2[3:6] - box1[3:6]
    q1 = Quaternion(box1[6:])
    q2 = Quaternion(box2[6:])
    qd = q2 / q1
    qd = np.array([qd.w, qd.x, qd.y, qd.z], dtype=np.float32)
    return np.concatenate([center_diff, size_diff, qd])

def apply_box_diff(box1, box_diff):
    center2 = box1[:3] + box_diff[:3]
    size2 = box1[3:6] + box_diff[3:6]
    q1 = Quaternion(box1[6:])
    qd = Quaternion(box_diff[6:])
    q2 = qd * q1
    q2 = np.array([q2.w, q2.x, q2.y, q2.z], dtype=np.float32)
    return np.concatenate([center2, size2, q2])
