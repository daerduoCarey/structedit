...
from data import SynShapesDiffDataset, Tree
import utils
...
def train(conf):
    ...
    train_dataset = SynShapesDiffDataset(conf.train_dataset, data_features, conf.shapediff_topk)
    train_dataloader = torch.utils.data.DataLoader(train_dataset, batch_size=conf.batch_size, shuffle=True, collate_fn=utils.collate_feats)
...
def forward(batch, data_features, encoder, decoder, device, conf,
    ...
    with torch.no_grad():
        ...
        if log_tb and tb_writer is not None:
            tb_writer.add_scalar(tag='loss/' + ('valdt' if is_valdt else 'train'), scalar_value=total_loss.item(), global_step=step)
            tb_writer.add_scalar(tag='lr/' + ('valdt' if is_valdt else 'train'), scalar_value=lr, global_step=step)
            tb_writer.add_scalar(tag='box_loss/' + ('valdt' if is_valdt else 'train'), scalar_value=losses['box'].item(), global_step=step)
            tb_writer.add_scalar(tag='leaf_loss/' + ('valdt' if is_valdt else 'train'), scalar_value=losses['leaf'].item(), global_step=step)
            tb_writer.add_scalar(tag='exists_loss/' + ('valdt' if is_valdt else 'train'), scalar_value=losses['exists'].item(), global_step=step)
            tb_writer.add_scalar(tag='semantic_loss/' + ('valdt' if is_valdt else 'train'), scalar_value=losses['semantic'].item(), global_step=step)
            tb_writer.add_scalar(tag='kldiv_loss/' + ('valdt' if is_valdt else 'train'), scalar_value=losses['kldiv'].item(), global_step=step)
            tb_writer.add_scalar(tag='l1/' + ('valdt' if is_valdt else 'train'), scalar_value=losses['kldiv'].item(), global_step=step)
            tb_writer.add_scalar(tag='diffnode_type_loss/' + ('valdt' if is_valdt else 'train'), scalar_value=losses['diffnode_type'].item(), global_step=step)
            tb_writer.add_scalar(tag='diffnode_box_loss/' + ('valdt' if is_valdt else 'train'), scalar_value=losses['diffnode_box'].item(), global_step=step)
