...
def draw_partnet_objects(
        objects, object_names=None, figsize=None, rep='boxes',
        leafs_only=False, use_id_as_color=False, visu_edges=True, sem_colors_filename=None, filename=None,
        hide_ticks=False, fid=0):

    if isinstance(objects, list):
        objects = np.array(objects).reshape(1, -1)
    if isinstance(object_names, list):
        object_names = np.array(object_names).reshape(1, -1)
    ...
    if figsize is not None:
        fig = plt.figure(fid, figsize=figsize)
    else:
        fig = plt.figure(fid)
    
    extent = 0.7
    for i, obj in enumerate(objects.reshape(-1)):
        if obj is not None:
            part_boxes, part_geos, edges, part_ids, part_sems = obj.graph(leafs_only=leafs_only)

            ax = fig.add_subplot(
                objects.shape[0], objects.shape[1], i+1, projection='3d')
            ...
            if object_names is not None and object_names.reshape(-1)[i] is not None:
                ax.set_title(object_names.reshape(-1)[i])
